...
export default function Admin() {
  const [authSession, setAuthSession] = useState({ isAuthenticated: false });
  ...
  // Sync with store (async)
  const refreshData = async () => {
    const [blogsData, regsData, eventsData, achData, settingsData, authData] = await Promise.all([
      Store.getBlogs(),
  ...
  useEffect(() => {
    const init = async () => {
      const auth = await Store.getAdminAuth();
      setAuthSession(auth);
  ...
  const handleSaveBlog = async (e) => {
    ...
    if (editingBlog) {
      await Store.updateBlog(editingBlog.id, blogForm);
      showToast('Blog article updated successfully!');
    } else {
      await Store.addBlog(blogForm);
  ...
  const handleStatusChange = async (id, newStatus) => {
    await Store.updateRegistrationStatus(id, newStatus);
  ...
  const handleSaveEvent = async (e) => {
    ...
    if (editingEvent) {
      await Store.updateEvent(editingEvent.id, eventForm);
  ...
  const handleSaveAchievement = async (e) => {
    ...
    await Store.addAchievement(achForm);
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-dark)' }}>
        <div style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
          <div className="admin-login-spinner" style={{ width: '40px', height: '40px', margin: '0 auto 16px' }} />
          Verifying session...
        </div>
      </div>
    );
      ...
      <div className="container">
        ...
        <div className="admin-header">
          ...
          <div className="admin-header__actions">
            ...
            <button
              className="btn-glass"
              style={{ padding: '10px 18px', fontSize: '0.88rem' }}
              onClick={async () => await Store.exportRegistrationsCSV()}
            >
              📥 Export CSV
            </button>
            <button
                      ...
                      <tr key={ev.id}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={async () => {
                              await Store.updateEvent(ev.id, { isLive: !ev.isLive });
        ...
        {activeTab === 'settings' && (
          <div className="admin-tab-content">
            ...
            <div className="admin-card-box">
              <h3 className="admin-card-box__title" style={{ marginBottom: '20px' }}>
                <span>🛡️</span> Administrator Security
              </h3>
              <div style={{ background: 'rgba(16, 185, 129, 0.08)', border: '1px solid rgba(16, 185, 129, 0.25)', borderRadius: '14px', padding: '20px' }}>
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: '1.6', margin: 0 }}>
                  🔒 Authentication is managed by <strong style={{ color: '#10B981' }}>Supabase Auth</strong>. To change your admin password or manage users, visit your
                  <a href="https://supabase.com/dashboard" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--orange)', marginLeft: '4px' }}>Supabase Dashboard → Authentication</a>.
                </p>
              </div>
            </div>
